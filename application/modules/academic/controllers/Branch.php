...
class Branch extends MY_Controller {
    ...
    public function index($school_id = null) {
        ...
        $this->data['classes'] = $this->branch->get_class_list($school_id);  
        
        $condition = array();
        $condition['status'] = 1;        
        if($this->session->userdata('role_id') != SUPER_ADMIN){            
            $condition['school_id'] = $this->session->userdata('school_id');        
            $this->data['teachers'] = $this->branch->get_list('teachers', $condition, '','', '', 'id', 'ASC');
        }   
        ...
        $this->layout->view('class/branch', $this->data);            
    ...
    public function add() {
        ...
        if ($_POST) {
            ...
            $this->_prepare_branch_validation();
           
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_class_data();
                
                $insert_id = $this->branch->insert('branch', $data);
                if ($insert_id) {
                    ...
                    redirect('academic/branch');
                } else {
                    error($this->lang->line('insert_failed'));
                    redirect('academic/branch/add');
                }
            } else {
                error($this->lang->line('insert_failed'));
        ...
        $this->data['classes'] = $this->branch->get_class_list();      
    ...
    public function edit($id = null) {       
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                $updated = $this->branch->update('branch', $data, array('id' => $this->input->post('id')));
                ...
                if ($updated) {
                 ...
                 error($this->lang->line('update_failed'));
                $this->data['class'] = $this->branch->get_single('classes', array('id' => $this->input->post('id')));
            }
        }
        
        if ($id) {
            $this->data['class'] = $this->branch->get_single('branch', array('id' => $id));

            if (!$this->data['class']) {
                 redirect('academic/classes/branch');
            }
        }
        $this->data['classes'] = $this->branch->get_class_list($this->data['branch']->school_id);   
     ...
     * @return          : null 
     * ********************************************************** */
    private function _prepare_branch_validation() {
        ...
        $this->form_validation->set_rules('password', $this->lang->line('password'), 'trim|required|callback_name');
    }
   ...
   {             
      ...
      {   
          $name = $this->branch->duplicate_check($this->input->post('school_id'), $this->input->post('name')); 
          if($name){
         ...
         $name = $this->branch->duplicate_check($this->input->post('school_id'), $this->input->post('name'), $this->input->post('id')); 
          if($name){
                $this->form_validation->set_message('name', $this->lang->line('already_exist'));         
                return FALSE;
          } else {
    ...
    public function delete($id = null) {
        ...
        $branch = $this->branch->get_single('branch', array('id' => $id));
        

        if ($this->branch->delete('branch', array('id' => $id))) {

            create_log('Has been deleted a class : '. $branch->name);            
