...
class Question extends MY_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('Question_Model', 'assignment', true);        
    }
    ...
    * ********************************************************** */
    public function index($class_id = null) {
        ...
        $this->data['questions'] = $this->assignment->get_assignment_list($class_id, $school_id, @$school->academic_year_id);
    ...
    public function add() {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                $insert_id = $this->assignment->insert('questions', $data);
                if ($insert_id) {
                    
                    create_log('Has been created an assignment : '.$data['title']); 
                    
                    success($this->lang->line('insert_success'));
                    redirect('exam/question/index/'.$data['class_id']);
                } else {
                    error($this->lang->line('insert_failed'));
                    redirect('exam/question/add/'.$data['class_id']);
                }
            } else {
                error($this->lang->line('insert_failed'));
        ...
        $this->data['questions'] = $this->assignment->get_assignment_list($class_id);
    ...
    * ********************************************************** */
    public function edit($id = null) {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_question_data();
                $updated = $this->assignment->update('questions', $data, array('id' => $this->input->post('id')));
                ...
                if ($updated) {
                    ...
                    redirect('exam/question/index/'.$data['class_id'].'/'.$data['school_id']);
                } else {
                    error($this->lang->line('update_failed'));
                    redirect('exam/question/edit/' . $this->input->post('id'));
                }
            } else {
                error($this->lang->line('update_failed'));
                $this->data['assignment'] = $this->assignment->get_single('questions', array('id' => $this->input->post('id')));
            }
        }

        if ($id) {
            $this->data['assignment'] = $this->assignment->get_single('question', array('id' => $id));
        ...
        $this->data['questions'] = $this->assignment->get_assignment_list($class_id, $this->data['assignment']->school_id, $this->data['assignment']->academic_year_id);
    ...
    * ********************************************************** */
    private function _prepare_assignment_validation() {
        ...
        $this->form_validation->set_rules('question', $this->lang->line('question'), 'trim');
        $this->form_validation->set_rules('option_a', $this->lang->line('option_a'), 'trim');
        $this->form_validation->set_rules('option_b', $this->lang->line('option_b'), 'trim');
        $this->form_validation->set_rules('option_c', $this->lang->line('option_c'), 'trim');
        $this->form_validation->set_rules('option_d', $this->lang->line('option_d'), 'trim');
    ...
    * ********************************************************** */
    public function delete($id = null) {
        ...
        $assignment = $this->assignment->get_single('questions', array('id' => $id));
        
        if ($this->assignment->delete('questions', array('id' => $id))) {
        ...
        redirect('exam/question/index/' . $assignment->class_id.'/'.$assignment->school_id);
    }
    ...
    public function get_assignment_by_section() {
        ...
        $school = $this->assignment->get_school_by_id($school_id); 
        $questions = $this->assignment->get_list('questions', array('class_id'=>$class_id, 'section_id'=>$section_id, 'academic_year_id'=>$school->academic_year_id), '','', '', 'id', 'ASC');
