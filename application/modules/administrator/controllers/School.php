...
class School extends MY_Controller {
    ...
    * ********************************************************** */
    public function index() {
        ...
        $this->data['schools'] = $this->school->get_list('schools', array('created_by' => logged_in_user_id()), '','', '', 'id', 'ASC');
    ...
    public function add() {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                $data['qrcode'] = sha1(rand().time()).'.png';
             
                $insert_id = $this->school->insert('schools', $data);

                if ($insert_id!='') {

            $this->qr_generator($insert_id,$data['qrcode']);
    ...
    * ********************************************************** */
    private function _prepare_school_validation() {
        ...
        $this->form_validation->set_rules('school_upi', $this->lang->line('school_upi'), 'trim|required');
    ...
    * ********************************************************** */
    ...
    private function _get_posted_school_data() {
     ...
     public function qr_generator($id,$name){
            $this->load->library('ciqrcode');
              ...
              $params['data']  = base_url()."school/".$id;
              // $params['level'] = $this->input->post('qr_level');
              $params['size'] = 10;
              $params['savename'] = FCPATH.'assets/uploads/school_qrcode/'.$name;
