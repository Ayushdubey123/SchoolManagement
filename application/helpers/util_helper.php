...
if (!function_exists('get_student_attendance_subject')) {

    function get_student_attendance_subject($student_id, $school_id, $academic_year_id, $class_id, $section_id, $year, $month, $day,$subject_id) {
        $ci = & get_instance();
        $field = 'day_' . abs($day);
        $ci->db->select('SA.' . $field);
        $ci->db->from('student_attendances AS SA');
        $ci->db->where('SA.student_id', $student_id);
        $ci->db->where('SA.academic_year_id', $academic_year_id);
        $ci->db->where('SA.subject_id', $subject_id);
        $ci->db->where('SA.school_id', $school_id);
        $ci->db->where('SA.class_id', $class_id);
...
if (!function_exists('get_paid_receipt_list_api')) {
function get_paid_receipt_list_api($school_id = null, $class_id = null, $section_id = null, $student_id = null, $academic_year_id = null, $txn_id = null){
        
        $ci = & get_instance();
        $ci->db->select('I.*, I.id as inv_id, I.discount AS inv_discount, T.*, T.id AS txn_id, E.section_id, SC.school_name, S.name AS student_name, S.present_address, S.phone, AY.session_year, C.name AS class_name');
        $ci->db->from('transactions AS T');        
        $ci->db->join('invoices AS I', 'I.id = T.invoice_id', 'left');
        $ci->db->join('classes AS C', 'C.id = I.class_id', 'left');
        $ci->db->join('students AS S', 'S.id = I.student_id', 'left');
        $ci->db->join('academic_years AS AY', 'AY.id = T.academic_year_id', 'left');
        $ci->db->join('schools AS SC', 'SC.id = T.school_id', 'left');
        $ci->db->join('enrollments AS E', 'E.id = I.student_id', 'left');
        ...
        if($academic_year_id){
            $ci->db->where('T.academic_year_id', $academic_year_id); 
            $ci->db->where('E.academic_year_id', $academic_year_id); 
        }
        

        $ci->db->where('T.id', $txn_id);
